...
async def calculate_accuracy(
    ...
    if confidence_level and confidence_level != "ALL":
        ...
        if confidence_level == "HIGH":
            ...
            query = query.where(
                and_(Prediction.confidence >= 0.5, Prediction.confidence < 0.7)
            )
    ...
    win_count = sum(
        1 for p in predictions if p.actual_outcome == "home" and p.home_win_prob > 0.5
    )
...
async def calculate_calibration_curve(
    ...
    for i in range(bins):
        ...
        bin_predictions = [
            p for p in predictions if bin_start <= p.home_win_prob <= bin_end
        ]
