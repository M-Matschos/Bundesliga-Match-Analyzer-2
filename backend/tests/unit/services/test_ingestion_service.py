...
def ingestion_service():
    ...
    return BundesligaDataIngestion(
        api_key="test_api_key", redis_client=mock_redis, poll_interval=10
    )
...
class TestBundesligaDataIngestion:
    ...
    async def test_event_deduplication(self, ingestion_service):
        ...
        # Generiere Hash
        event_hash = ingestion_service._generate_event_hash(
            "goal", match_id, event_data
        )
    ...
    async def test_extract_stat_from_api_response(self, ingestion_service):
        ...
        possession = BundesligaDataIngestion._extract_stat(
            stats_dict, "Ball Possession"
        )
    ...
    async def test_extract_missing_stat(self, ingestion_service):
        ...
        stats_dict = {"statistics": [{"type": "Shots on Goal", "value": "5"}]}
...
class TestEventProcessing:
    ...
    async def test_process_goal_event(self, mock_publish, ingestion_service):
        ...
        # Verarbeite Event
        count = await ingestion_service.process_match_events_from_api(
            match_id, 1, [api_event]
        )
